...
# sliding_window_max_sub_array_sum
# Given an array of integers and a number, write a function called max_sub_array_sum, which finds the maximum sum of a subarray with the length of the number passed to the function.
...
def max_sub_array_sum(arr, size_of_sub)
    return nil if arr.length < size_of_sub
    max_sum = 0
    arr.each_with_index.reduce(0) do |total, (elem, i)|
        total += elem
        if i > size_of_sub - 1
            total -=  arr[i - size_of_sub]
            max_sum = total if total > max_sum
        end
        puts "t = #{total}, m = #{max_sum}, i = #{i}, curr = #{elem}"
        total
    end
    max_sum
end
...
# function max_sub_array_sum(arr, slide) {
#     let current_sum = 0;
...
# function max_sub_array_sum(arr, num){
